...
def classify_minilearn_intent(text: str) -> str:
    """Return a coarse label only; callers must not persist inbound raw text."""
...
class PvResponseMemoryShadow:
    """Small, fail-closed learner inside the existing PV rib."""
    ...
    async def _is_writer_effect(self, *, peer: int, message_id: int, text: str) -> bool:
        """Fail closed: exact message id wins; recent peer+text effect is fallback.

        Effects are journaled before the Telegram mutation, so the fallback does
        not depend on the result row having been completed yet. False negatives
        would teach automation, so any matching evidence is treated as automatic.
        """
                ...
                """SELECT EXISTS(
                     ...
                     WHERE te.created_at > NOW()-INTERVAL '5 minutes'
                       ...
                       AND (
                         ...
                         OR (
                           te.payload->>'peer'=$1::TEXT
                           AND COALESCE(te.payload->>'text','')=$3
                         )
                       )
                   )""",
                int(peer),
    ...
    async def observe_outbound(
        ...
        *,
        ...
        row = await self.pool.fetchrow(
            """SELECT intent FROM pv_minilearn_context
               WHERE user_id=$1 AND observed_at > NOW()-INTERVAL '24 hours'""",
            int(user_id),
        )
        if row is None:
        ...
        await self.pool.execute(
            """INSERT INTO pv_minilearn_memory(
                 intent,normalized_response,response_text,times_seen,first_seen_at,last_seen_at)
