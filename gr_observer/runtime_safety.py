"""Production traffic safety and passive-signal runtime policy.

This layer keeps the modular monolith topology intact: one Telegram user
session, one Outbox and one Writer.  It adds three production policies without
creating another client or worker topology:

* Radar is event-driven when enabled; its legacy periodic crawler is not started.
* Telegram mutations are governed by durable FloodWait cooldowns.
* useful Telegram links/signals are persisted at the spine, so PV/Groups never
  depend on the Radar module being enabled.
"""
...
log = logging.getLogger("gr-observer.runtime-safety")
...
class DeferredTraffic(BaseException):
    """Control-flow signal: Telegram work was durably deferred, not failed.

    BaseException is intentional.  Feature modules commonly catch Exception to
    translate business failures; a governor deferral must cross those handlers
    unchanged until the single Writer can park the action safely.
    """
...
def telegram_links(text: str) -> tuple[str, ...]:
    """Extract Telegram links locally, without resolving/opening them."""
    found: list[str] = []
    seen: set[str] = set()
...
class SignalLedger:
    """Neutral, local event ledger shared by ribs through the spine only."""
    ...
    async def ensure_schema(self) -> None:
        ...
        async with self._lock:
            if self._ready:
                return
            await self.pool.execute(SAFETY_SCHEMA)
            self._ready = True
    ...
    async def observe_event(self, event, me) -> None:
        """Record high-value facts using only data already present in the update."""
        await self.ensure_schema()
        ...
        # Origin correlation lives at the spine, not inside Radar.  Therefore a
        # private conversation remains fully functional when Radar is off.
...
class TrafficGovernor:
    """Durable account/effect/peer cooldowns driven by Telegram feedback."""
    ...
    async def record_flood_wait(
        ...
        *,
        ...
        """Persist Telegram's wait and requeue without sleeping inside Writer."""
        await self.ensure_schema()
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                        ...
                        """INSERT INTO traffic_cooldowns(
                           ...
                           ON CONFLICT(scope,scope_key) DO UPDATE SET
                             blocked_until=GREATEST(
                               traffic_cooldowns.blocked_until,
                ...
                # FLOOD_WAIT is a conclusive rejection of this invocation.  It
                # is safe to remove only this in-progress effect journal row so
                # the same deterministic effect can be retried after cooldown.
                await conn.execute(
                    "DELETE FROM telegram_effects WHERE effect_key=$1 AND status='processing'",
                    effect_key,
                )
                await conn.execute(
                    """UPDATE outbox_actions SET status='pending',
    ...
    async def note_success(self, effect_type: str, payload: dict[str, Any]) -> None:
        """Decay historical strike counters only after their cooldown expired."""
        if effect_type == "send_panel_text":
            return
        await self.ensure_schema()
        peer_key = self._peer_key(payload)
...
class SafeTelegramEffects(TelegramEffects):
    """TelegramEffects with durable governor semantics and no FloodWait loop."""
    ...
    def __init__(self, *args, governor: TrafficGovernor, **kwargs) -> None:
    ...
    async def perform(
        ...
        # Mutations that call perform() directly (join/leave/callback etc.) must
        # also pass through the one-finger write pacer.  Existing send helpers
        # pace themselves immediately before their Telegram request.
        if (
        ...
        try:
            ...
            log.warning(
                "TrafficGovernor deferiu FloodWait efeito=%s tipo=%s por %ss",
                effect_key,
                effect_type,
                wait_seconds,
            )
            raise DeferredTraffic(
                wait_seconds,
                f"FloodWait {effect_type}: {wait_seconds}s",
            )
...
class SafeOutboxWriter(OutboxWriter):
    """Single Writer with durable FloodWait deferral and shared governor."""
    ...
    async def run(self) -> None:
        ...
        while not self._stopped.is_set():
            # A server-directed account cooldown must survive Railway restarts.
...
class ProductionSafetyMixin:
    """Explicit production policy mixed into Railway's existing Observer."""
    ...
    async def _connect_module(self, module_id: str) -> None:
        ...
        # Preserve durable-peer infrastructure before connecting any rib.
        installer = getattr(self, "_install_durable_peer_resolver", None)
        if installer is not None:
            await installer()
        if module_id in self.connected_modules or self.user is None:
            return
        item = self.registry.get(module_id)
        radar = item.implementation
        # Passive mode: receive NewMessage updates but never start scan_loop.
        radar.client = self.user
        radar.me = self.me
        radar.scan_task = None
        self.connected_modules.add(module_id)
        item.reason = "Ligado — passivo, sem varredura periódica"
        await self.storage.set_module_reason(module_id, item.reason)
        self.enabled = True
        self.state_reason = item.reason
        log.info("Radar conectado em modo passivo; crawler periódico desativado")

    async def guarded_dispatch(self, event) -> None:
        try:
            ledger = getattr(self, "signal_ledger", None)
            if ledger is not None and self.me is not None:
                await ledger.observe_event(event, self.me)
        except Exception as exc:
    ...
    async def user_runtime(self) -> None:
        """Observer.user_runtime with the same topology and a safer Writer."""
        session_lock = None
        ...
        try:
            ...
            self.user.add_event_handler(self.guarded_dispatch, __import__("telethon").events.NewMessage())
