...
class MiniLearnIntentTests(unittest.TestCase):
    ...
    def test_context_schema_has_no_inbound_text_column(self):
        context_ddl = MINILEARN_SCHEMA.split("CREATE TABLE IF NOT EXISTS pv_minilearn_memory", 1)[0]
        for forbidden in ("raw_text", "message_text", "inbound_text"):
            self.assertNotIn(forbidden, context_ddl)
...
class MiniLearnShadowTests(unittest.IsolatedAsyncioTestCase):
    def memory(self, *, enabled=True):
        pool = NS(execute=AsyncMock(), fetchrow=AsyncMock(), fetchval=AsyncMock())
    ...
    async def test_inbound_is_reduced_to_intent_before_storage(self):
        ...
        intent = await memory.observe_inbound(user_id=10, message_id=20, text="qual o valor do vip?")
    ...
    async def test_unknown_context_does_not_learn(self):
        ...
        result = await memory.observe_outbound(user_id=10, message_id=31, text="claro", has_media=False)
        self.assertFalse(result.learned)
    ...
    async def test_media_is_ignored_before_database_lookup(self):
        memory, pool = self.memory()
        result = await memory.observe_outbound(user_id=10, message_id=31, text="foto", has_media=True)
        self.assertFalse(result.learned)
